...
- [x] AC-US1-01: `scanRepositories()` sets `isUmbrellaManaged: true` on repos that lack own `.specweave/` but are under an umbrella project that has one.
- [x] AC-US1-02: Badge shows "SpecWeave" (green) for own `.specweave/`, "via umbrella" (info/cyan) for umbrella-managed, "No SpecWeave" (gray) otherwise.
- [x] AC-US1-03: "With SpecWeave" KPI counts repos with either `hasSpecweave === true` or `isUmbrellaManaged === true`.
