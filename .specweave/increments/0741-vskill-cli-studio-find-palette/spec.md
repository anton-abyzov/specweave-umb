...
Port the Find/Search palette into the local vskill studio CLI (eval-ui) with Cmd+Shift+K shortcut, in-eval-ui skill detail view with version selector and one-click install, and extend eval-server platform-proxy to forward /api/v1/studio/search, /api/v1/studio/telemetry/*, /api/v1/stats to upstream verified-skill.com (overridable via VSKILL_PLATFORM_URL).
...
To complete this specification, run:
  Tell Claude: "Complete the spec for increment 0741-vskill-cli-studio-find-palette"
