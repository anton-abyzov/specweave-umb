...
**Acceptance Criteria**:
- [ ] **AC-US1-01**: `specweave hook <event-type>` CLI command exists and routes to correct handler
- [ ] **AC-US1-02**: Hook router reads JSON from stdin and writes JSON to stdout (Claude Code hook protocol)
- [ ] **AC-US1-03**: Router returns safe JSON on any error -- `{"continue":true}` for session/compact hooks, `{"decision":"allow"}` for pre-tool-use, `{"decision":"approve"}` for prompt/stop hooks
- [ ] **AC-US1-04**: Dynamic imports used per handler for fast startup (no loading all handlers upfront)
- [ ] **AC-US1-05**: Command is hidden from `specweave --help` output
...
**Acceptance Criteria**:
- [ ] **AC-US2-01**: Writes `context-pressure.json` to `.specweave/state/` with escalating levels (elevated/critical/emergency)
- [ ] **AC-US2-02**: Writes `prompt-health-alert.json` to `.specweave/state/` with remediation advice
- [ ] **AC-US2-03**: Returns `{"continue":true}` when no `.specweave/config.json` exists (non-SpecWeave projects pass through)
- [ ] **AC-US2-04**: Respects `SPECWEAVE_DISABLE_HOOKS=1` environment variable (returns safe default immediately)
...
**Acceptance Criteria**:
- [ ] **AC-US3-01**: Edit/Write on `metadata.json` that changes status to "completed" is blocked (must use `specweave complete`)
- [ ] **AC-US3-02**: Write to `spec.md` blocked when strict interview is incomplete (checks `metadata.json` interview state)
- [ ] **AC-US3-03**: TeamCreate blocked when no qualifying active increment exists (status "active" or "in-progress")
- [ ] **AC-US3-04**: Non-increment file paths (files outside `.specweave/increments/`) always return `{"decision":"allow"}`
- [ ] **AC-US3-05**: Returns well-formed `{"decision":"allow"}` or `{"decision":"block","reason":"..."}` JSON in all cases
...
**Acceptance Criteria**:
- [ ] **AC-US4-01**: Detects `metadata.json` status changes and queues lifecycle events (increment.started, increment.paused, etc.)
- [ ] **AC-US4-02**: Detects `spec.md` AC checkbox changes (unchecked to checked and vice versa)
- [ ] **AC-US4-03**: Queues events to `.specweave/state/event-queue/pending.jsonl` for stop-sync to process
- [ ] **AC-US4-04**: Triggers immediate sync for `increment.done` and `increment.reopened` events (does not wait for stop-sync)
- [ ] **AC-US4-05**: Analytics handler tracks Skill/Task tool invocations to `.specweave/state/analytics/events.jsonl`
- [ ] **AC-US4-06**: GitHub/JIRA/ADO integration handlers loaded lazily based on `sync` config in `.specweave/config.json`
...
**Acceptance Criteria**:
- [ ] **AC-US5-01**: Clears stale auto-mode session files (`.specweave/state/auto-session-*.json` older than 24h)
- [ ] **AC-US5-02**: Resets context pressure state (clears `context-pressure.json`)
- [ ] **AC-US5-03**: Performs baseline health check -- validates CLAUDE.md exists, checks MEMORY.md size warning
- [ ] **AC-US5-04**: Returns `{"continue":true}` when no `.specweave/` directory exists (non-SpecWeave projects pass through)
...
**Acceptance Criteria**:
- [ ] **AC-US6-01**: Built-in Claude Code commands (`/help`, `/clear`, `/config`, `/status`) skip SpecWeave processing entirely
- [ ] **AC-US6-02**: `/sw:` prefixed commands get fast-path routing (no LLM intent detection needed)
- [ ] **AC-US6-03**: Project scope guard blocks SpecWeave skills outside initialized projects (no `.specweave/` directory)
- [ ] **AC-US6-04**: Intent detection delegates to existing `detect-intent` CLI command (no reimplementation)
- [ ] **AC-US6-05**: Context injection appends active increment info (ID, status, current task) to prompt
- [ ] **AC-US6-06**: Turn deduplication via content hash prevents duplicate processing of identical prompts
- [ ] **AC-US6-07**: Returns `{"decision":"approve"}` with optional `user_prompt` field, or `{"decision":"block","reason":"..."}`
...
**Acceptance Criteria**:
- [ ] **AC-US7-01**: `stop-reflect` extracts learnings from conversation when `reflect.enabled: true` in config
- [ ] **AC-US7-02**: `stop-reflect` returns immediately when disabled in config or no `.specweave/` exists
- [ ] **AC-US7-03**: `stop-auto` validates completion gates -- checks task completion count, session age, and auto-session file
- [ ] **AC-US7-04**: `stop-sync` reads `pending.jsonl`, deduplicates by event type + increment ID, syncs to external tools
- [ ] **AC-US7-05**: All stop hooks return `{"decision":"approve"}` -- never block session exit regardless of errors
...
**Acceptance Criteria**:
- [ ] **AC-US8-01**: `hooks.json` updated to use `specweave hook <event>` commands instead of `bash .../run-hook.sh`
- [ ] **AC-US8-02**: All 59 `.sh` files, 1 `.mjs` file, 2 `.cmd` files, and 1 `.ps1` file deleted from `hooks/`
- [ ] **AC-US8-03**: All subdirectories deleted: `universal/`, `lib/`, `v2/` (with all nested dirs), `_archive/`, `tests/`
- [ ] **AC-US8-04**: `README.md` deleted from `hooks/`
- [ ] **AC-US8-05**: Only `hooks.json` remains in the `hooks/` directory
...
**Acceptance Criteria**:
- [ ] **AC-US9-01**: `copyPluginSkillsToProject` in `src/utils/plugin-copier.ts` no longer creates `.claude/hooks/` directory or copies hook files
- [ ] **AC-US9-02**: Skills are still copied to `.claude/skills/` correctly (no regression)
- [ ] **AC-US9-03**: Lockfile entries in `vskill.lock` are still computed correctly (hash, version, timestamp)
