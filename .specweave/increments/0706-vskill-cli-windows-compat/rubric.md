...
- [x] `sw:code-reviewer` passes with 0 critical/high/medium findings. — PASS 2026-04-24 (re-review): 0 critical, 0 high, 0 medium, 3 low, 2 info. Previous F-01 resolved in commit ad6a24a (install-smoke uses `-y` flag + exits on crash codes). Additional hardening 71deceb (output-shape grep check to catch silent exit-0 crashes). See `reports/code-review-report.json`.
