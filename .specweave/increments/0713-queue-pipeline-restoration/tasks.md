...
> **Scope note (2026-04-25):** The original 25-task plan included 6 production-execution tasks (deploy + drain + backfill + verify) that require operator credentials. Those tasks (formerly T-005, T-008, T-012, T-013, T-017, T-024) were split out into **0713B-production-rollout** so 0713 can close at "implementation complete" without blocking on the production calendar. Task IDs below are non-contiguous on purpose to preserve traceability with git history; there is no T-005, T-008, T-012, T-013, T-017, or T-024 in this file.
...
**User Story**: All | **Satisfies ACs**: regression | **Status**: [x] completed
- `npx vitest run` — 0713-domain: 51/51 pass across 7 files (queue-stats-refresh, route.failure-surface, state-history, drain-stuck-received, stale-received-recovery, choose-boot-candidates, backfill-state-history). Full suite shows 3708 pass / 150 pre-existing failures (jose `payload must be an instance of Uint8Array` JWT signing in admin auth tests — tracked in 0690).
- `npx playwright test tests/e2e/queue*.spec.ts` — 0713-domain: 5/5 new tests pass (queue-default-sort 2/2, queue-degraded-no-flip 3/3). Pre-existing flake in queue-pagination, queue-stat-card-stability, queue-submit-visibility, queue-duplicates verified against baseline (stash) — same failures occur without 0713 changes.
- Pre-existing failures flagged for PR description; not regressions.
