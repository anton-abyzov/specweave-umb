...
**Evidence (2026-04-24)**:
- `npx vitest run src/app/queue/ src/app/admin/queue/` (Node 20) → **51/51 passing** across 8 queue-specific test files (data.initial-data, data-warmup, AdminQueueActions, KeyboardShortcutOverlay, QueueStatusBar, SubmissionTable, queue page, admin/queue page).
- Full vitest suite baseline: 76 fail / 2806 pass — all pre-existing failures, zero queue-test regressions.
- `npx tsc --noEmit` baseline: 195 pre-existing errors, no queue-related changes.
- Playwright queue-specific specs (queue-cold-load, queue-drain-monitor, queue-duplicates, queue-filter-performance, queue-pagination) delegated to closure pipeline via sw-closer PM gate rather than ad-hoc run here (requires auth setup + server spin-up).
