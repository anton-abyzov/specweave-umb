...
**Status**: Completed
**Satisfies ACs**: AC-US1-01, AC-US1-04, AC-US1-05, AC-US4-05
**Test**: Given free user → response has `skillLimit: null` (was `50`). Pro/enterprise unchanged (still `null`). Vitest covers the 3 tier shapes. AC-US1-04 (cache eventual consistency) and AC-US1-05 (telemetry preserved via `/quota/report`) are non-regression observational ACs covered by the same server-side change — `quota-report.test.ts` already asserts `lastReportedSkillCount` updates correctly.
