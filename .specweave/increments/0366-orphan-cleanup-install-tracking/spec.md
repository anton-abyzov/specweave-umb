...
**Acceptance Criteria**:
- [x] **AC-US1-01**: When a submission is created for a repo+skillName that already has a published Skill record, the old Skill record gets `isDeprecated = true` within the same transaction
- [x] **AC-US1-02**: Orphan cleanup uses one `findMany` + one `updateMany` Prisma query (no N+1)
- [x] **AC-US1-03**: Cleanup runs inside the existing batch submission flow in `POST /api/v1/submissions` -- no separate cron job
- [x] **AC-US1-04**: Skills that are NOT being re-submitted are never touched by the cleanup logic
- [x] **AC-US1-05**: The cleanup is idempotent -- running it multiple times for the same submission produces the same result
...
- [x] **AC-US2-05**: Setting `VSKILL_NO_TELEMETRY=1` disables the phone-home entirely
- [x] **AC-US2-06**: The platform endpoint increments `Skill.vskillInstalls` by 1 for the named skill
- [x] **AC-US2-07**: The platform endpoint is rate-limited at 60 requests/hour per IP using existing `checkRateLimit()` from `src/lib/rate-limit.ts` with `RATE_LIMIT_KV`
- [x] **AC-US2-08**: The platform endpoint returns 404 if the skill does not exist
- [x] **AC-US2-09**: No deduplication of installs -- simple counter increment
