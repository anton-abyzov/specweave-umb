...
export const VSKILL_BIN = join(REPO_ROOT, "repositories/anton-abyzov/vskill/dist/bin.js");
...
/** Pick a free TCP port to avoid conflicts with the user's running studios. */
export function pickPort() {
  // 5854 + random-ish offset (avoid collision with the dashboard server on 5853)
  return 5900 + Math.floor(Math.random() * 100);
}
