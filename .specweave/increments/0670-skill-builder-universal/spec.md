...
- [x] **AC-US2-02** *(SKILL.md instructions + detection script shipped; live browser flow re-verification deferred per Anton signoff 2026-05-09)*: When the CLI is absent but Node + vskill package is present, the SKILL.md instructs the agent to start `vskill eval serve` and walk the user through the existing browser Studio UI (path B — uses the 0465 workspace).
