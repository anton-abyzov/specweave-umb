...
- [ ] **AC-US2-04**: Path selection is described in `references/fallback-modes.md` with the exact detection commands (`which vskill`, `node -e "require.resolve('vskill')"`, `test -d ~/.claude/skills/skill-creator`) and the exact warning messages for each transition.
- [ ] **AC-US2-05**: Path selection is tested via the detection script, not by parsing markdown. A Vitest test invokes the detection logic with mocks for (a) CLI present → asserts path A, (b) CLI absent + package resolvable → asserts path B, (c) both absent + skill-creator installed → asserts path C + warning, (d) all absent → asserts a clear error exit with install instructions.
...
- [ ] **AC-US3-04**: When `--targets` is omitted, defaults to the 7 SKILL.md-native universal agents: `claude-code, cursor, github-copilot, windsurf, codex, gemini-cli, cline`.
...
- [ ] **AC-US3-07**: `vskill skill list` delegates to existing `vskill list` (thin alias). `vskill skill info <name>` reads the skill's frontmatter and divergence report if present.
- [ ] **AC-US3-08**: `vskill skill publish <name>` is an alias for `vskill submit <name>` (no new code path — delegates to the existing submission flow).
- [ ] **AC-US3-09**: `registerSkillCommand(program)` is invoked in `src/cli/index.ts` alongside `add`, `init`, `submit`.
...
**Acceptance Criteria**:
- [ ] **AC-US4-01**: `src/core/skill-generator.ts` exists, exporting a pure async function `generateSkill(request: GenerateSkillRequest): Promise<GenerateSkillResult>` with no HTTP-layer dependencies (no `req`/`res`, no SSE).
- [ ] **AC-US4-02**: The extracted function preserves the behavior of `src/eval-server/skill-create-routes.ts:919-1100` (pre-extraction) — asserted by a snapshot test on 3 target combinations (Claude only, Claude+Codex+Cursor, all 7 universal).
- [ ] **AC-US4-03**: `src/eval-server/skill-create-routes.ts` at the `/api/skills/generate` handler is refactored to a thin wrapper: accept request, handle SSE, call `generateSkill()`, stream results back. Total handler body ≤ 40 lines.
- [ ] **AC-US4-04**: `buildAgentAwareSystemPrompt` (existing from 0665) continues to function identically — called from inside `generateSkill()` rather than inline in the HTTP handler.
- [ ] **AC-US4-05**: No regression in existing Skill Studio UI — browser users see the same generated output before and after extraction (verified by keeping existing eval-ui tests green).
...
- [ ] **AC-US6-04**: A unit test asserts the field appears in emitted SKILL.md for all 7 universal targets.
...
- [ ] **AC-US7-01**: `tests/integration/skill-cli.test.ts` (Vitest) exists and runs via `npx vitest run tests/integration/skill-cli.test.ts`. Tests CLI-only assertions.
- [ ] **AC-US7-02**: The Vitest integration test creates a sandbox dir via `mkdtemp`, runs `vskill install anton-abyzov/vskill/plugins/skills/skills/skill-builder`, and asserts the skill lands in at least `.claude/skills/skill-builder/` AND `.agents/skills/skill-builder/`.
- [ ] **AC-US7-03**: The Vitest integration test runs `vskill skill new --prompt "lint markdown files" --targets=claude-code,codex,cursor,opencode` and asserts SKILL.md exists at all four target paths.
- [ ] **AC-US7-04**: The Vitest integration test asserts every emitted SKILL.md contains `x-sw-schema-version: 1` in its frontmatter (parsed with a YAML loader, not a regex).
- [ ] **AC-US7-05**: The Vitest integration test asserts `lint-markdown-files-divergence.md` exists in sandbox cwd and, when the source had `allowed-tools: [Bash]`, asserts the OpenCode entry reads `allowed-tools → permission: { bash: ask }` (positive case). A second test case asserts that emitting to OpenCode with `allowed-tools` set and omitting the divergence entry is a test failure (negative case — protects against silent loss).
...
- [ ] **AC-US7-07**: `tests/e2e/skill-studio-regression.spec.ts` (Playwright) exists and verifies that after T-002's HTTP-handler rewire, the existing Skill Studio browser flow (path B) still renders, generates, and emits correctly. Covers `POST /api/skills/generate` via the UI — no CLI assertions.
...
- [ ] **AC-US8-03**: README.md badge count updated from `skills: 7` to `skills: 8`, and CHANGELOG.md has a `### Added` entry for the new `vskill skill` subcommand plus the bundled `skill-builder` skill.
- [ ] **AC-US8-04**: README.md "Commands" or equivalent section includes a `vskill skill new|import|list|info|publish` subsection with a usage example (`vskill skill new --prompt "lint markdown" --targets=claude-code,codex`). `--help` output alone does not satisfy this AC.
...
- [ ] **AC-US9-03**: Umbrella sync commit lands on the umbrella repo main branch following the established pattern (`sync umbrella after vskill v0.5.X release`).
- [ ] **AC-US9-04**: No SpecWeave release required for this increment (deliberate — confirms the no-SpecWeave-touch MVP scope).
...
- `vskill skill new --prompt "X"` emits to 7 universal targets + divergence report in under 30 seconds
