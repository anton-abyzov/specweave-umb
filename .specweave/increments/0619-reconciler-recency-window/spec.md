...
**Acceptance Criteria**:
- [ ] **AC-US1-01**: `bulkFetchIssueStates()` method added to GitHubClientV2 that returns `Map<number, 'open' | 'closed'>` from a single `gh search issues` call
- [ ] **AC-US1-02**: `reconcileIssue()` reads issue state from pre-fetched map instead of calling `getIssue()` per issue
- [ ] **AC-US1-03**: If an issue is not found in the bulk map (edge case), falls back to individual `getIssue()` call
- [ ] **AC-US1-04**: Bulk search is limited to 100 issues by default (configurable via `--full` for no limit)
...
**Acceptance Criteria**:
- [ ] **AC-US2-01**: Milestone fetch uses `per_page=20&sort=updated&direction=desc` instead of `--paginate`
- [ ] **AC-US2-02**: `--full` flag restores `--paginate` behavior for deep reconciliation
...
**Acceptance Criteria**:
- [ ] **AC-US3-01**: `full` option added to `ReconcileOptions` interface
- [ ] **AC-US3-02**: CLI `github-reconcile --full` passes `full: true` to reconciler
- [ ] **AC-US3-03**: When `full: true`, bulk search has no limit and milestones use `--paginate`
