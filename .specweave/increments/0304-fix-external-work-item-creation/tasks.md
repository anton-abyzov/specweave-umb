...
- [ ] Add import for `isTemplateFile` from `../../core/increment/template-creator.js`
- [ ] After `detectProvider()` check, add template guard: `isTemplateFile(specPath)` returns early
...
**Implementation Steps**:
- [ ] Create or update test file for `ExternalIssueAutoCreator`
- [ ] Test: template spec.md -> skipped, no API calls
- [ ] Test: real spec.md -> proceeds normally (mock API calls)
...
**Implementation Steps**:
- [ ] Add `sync_living_docs?: boolean` to `HookConfiguration.post_increment_planning` in `src/core/config/types.ts`
- [ ] Add `sync_living_docs` property to `post_increment_planning` in `src/core/schemas/specweave-config.schema.json`
...
**Implementation Steps**:
- [ ] Refactor `onIncrementPlanned()` to read the full `post_increment_planning` config
- [ ] Add living docs sync before external issue creation
- [ ] Ensure errors in living docs sync don't prevent external issue creation
...
**Implementation Steps**:
- [ ] Update `src/templates/config.json.template`
- [ ] Update `src/cli/helpers/init/directory-structure.ts`
- [ ] Update `src/cli/helpers/issue-tracker/sync-config-writer.ts`
- [ ] Update `src/cli/helpers/init/smart-defaults.ts`
...
**Implementation Steps**:
- [ ] Add test: `onIncrementPlanned` calls `LivingDocsSync.syncIncrement` when `sync_living_docs: true`
- [ ] Add test: `onIncrementPlanned` calls living docs sync BEFORE external issue creation
- [ ] Add test: `onIncrementPlanned` still works when `sync_living_docs: false` (backward compat)
...
**Implementation Steps**:
- [ ] Run `npm test` in the specweave repository
- [ ] Fix any failures
...
**By User Story**:
- US-001: 0/2 tasks completed (0%)
- US-002: 0/3 tasks completed (0%)
- US-003: 0/2 tasks completed (0%)

**Overall**: 0/7 tasks completed (0%)
