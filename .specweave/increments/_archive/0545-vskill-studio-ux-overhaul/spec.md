...
**Acceptance Criteria**:
- [x] **AC-US1-01**: Given a pass rate value of 0.85, when the color threshold logic evaluates it, then the color maps to green (>= 0.7 threshold on 0-1 scale)
- [x] **AC-US1-02**: Given a pass rate value of 0.45, when the color threshold logic evaluates it, then the color maps to yellow (>= 0.4 and < 0.7 on 0-1 scale)
- [x] **AC-US1-03**: Given a pass rate value of 0.15, when the color threshold logic evaluates it, then the color maps to red (< 0.4 on 0-1 scale)
...
**Acceptance Criteria**:
- [x] **AC-US2-01**: Given a history timeline with multiple runs, when rendered, then the leftmost point is the oldest run and the rightmost is the newest
- [x] **AC-US2-02**: Given a run on the timeline, when displayed, then it shows duration in seconds, total tokens used, and model name as metadata
- [x] **AC-US2-03**: Given a point on the timeline, when clicked, then the detail panel navigates to that run's full results view
- [x] **AC-US2-04**: Given a timeline with 20+ runs, when rendered, then the axis labels remain readable with no overlap (auto-spacing or scroll)
...
**Acceptance Criteria**:
- [x] **AC-US3-01**: Given an activation in progress, when 120 seconds elapse without completion, then the activation terminates and displays a timeout message
- [x] **AC-US3-02**: Given an activation in progress, when the user clicks the Cancel button, then the activation stops and results for prompts that completed before cancellation are displayed
- [x] **AC-US3-03**: Given a cancelled activation where 3 of 5 prompts completed, when the partial results are shown, then each completed prompt displays its verdict and score, and incomplete prompts show a "Cancelled" status
- [x] **AC-US3-04**: Given an activation in progress, when rendered, then a Cancel button is visible alongside a progress indicator showing completed/total prompts
...
**Acceptance Criteria**:
- [x] **AC-US4-01**: Given a file selected in the browser, when it is a JSON file, then it renders with syntax-highlighted formatted JSON in the preview pane
- [x] **AC-US4-02**: Given any file in the skill directory, when the user clicks Edit, then a text editor opens with the file contents and a Save button that issues a PUT request to persist changes
- [x] **AC-US4-03**: Given a file listing, when displayed, then each file shows its size in human-readable format (bytes, KB, MB)
- [x] **AC-US4-04**: Given a user edits a file and clicks Save, when the PUT request succeeds, then a success toast appears and the preview refreshes with the updated content
...
**Acceptance Criteria**:
- [x] **AC-US5-01**: Given an evals.json entry, when it contains an optional `testType` field set to "unit" or "integration", then the UI displays the corresponding badge next to the eval name
- [x] **AC-US5-02**: Given an evals.json entry with no `testType` field, when rendered, then it defaults to displaying a "unit" badge (backward compatible)
- [x] **AC-US5-03**: Given the eval list view, when the user selects a test type filter (All / Unit / Integration), then only evals matching the filter are displayed
- [x] **AC-US5-04**: Given an integration test that requires credentials, when the required environment variables are not set, then the Run button is disabled and a message lists the specific missing credentials
- [x] **AC-US5-05**: Given an integration test with all required credentials configured, when the user clicks Run, then the eval executes normally
...
**Acceptance Criteria**:
- [x] **AC-US6-01**: Given two or more model runs selected for comparison, when the comparison view renders, then each rubric criterion shows per-model scores in a horizontal bar or radar chart
- [x] **AC-US6-02**: Given a comparison of model outputs for a prompt, when displayed side-by-side, then each output shows the first 500 characters with a "Show full output" toggle that expands to the complete text
- [x] **AC-US6-03**: Given a comparison across all prompts, when a model has the highest aggregate score, then a "Winner" badge appears next to that model's name in the summary header
- [x] **AC-US6-04**: Given a comparison view, when rendered, then an aggregate chart (bar chart) shows overall pass rate, average score, and average duration per model
- [x] **AC-US6-05**: Given two model outputs displayed side-by-side, when both are visible, then text differences are not highlighted (plain text display, not a diff view)
...
**Acceptance Criteria**:
- [x] **AC-US7-01**: Given a verdict cell in the results table, when the user hovers over it, then a tooltip displays the explanation text from `verdictExplanation()`
- [x] **AC-US7-02**: Given a `verdictExplanation(verdict, score, rubric)` helper function, when called with a PASS verdict and score >= 0.7, then it returns a string explaining which rubric criteria were met
- [x] **AC-US7-03**: Given a `verdictExplanation()` call with a FAIL verdict and score < 0.4, then it returns an explanation listing failed criteria and a recommendation string suggesting specific improvements
- [x] **AC-US7-04**: Given an eval result with verdict INEFFECTIVE (score < 0.2), when displayed, then a "Recommendations" section appears below the result with actionable suggestions (e.g., "Consider adding examples to your system prompt")
