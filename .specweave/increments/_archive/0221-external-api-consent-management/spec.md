...
**Acceptance Criteria**:
- [x] **AC-US1-01**: Free providers (claude-code, ollama) bypass consent — no prompt needed
- [x] **AC-US1-02**: External providers (anthropic, openai, azure-openai, bedrock, vertex-ai) require consent check
- [x] **AC-US1-03**: `checkConsent()` reads `.specweave/config.json` `externalModels` section
- [x] **AC-US1-04**: When `consent: "always-allow"`, all external providers are granted
- [x] **AC-US1-05**: When `consent: "never"`, all external providers are denied
- [x] **AC-US1-06**: When `consent: "ask"` and provider in `allowedProviders`, consent is granted
- [x] **AC-US1-07**: When `consent: "ask"` and provider NOT in `allowedProviders`, returns `'ask'` status
...
**Acceptance Criteria**:
- [x] **AC-US2-01**: `grantStandingConsent()` adds provider to `allowedProviders` in config.json
- [x] **AC-US2-02**: Duplicate providers are not added
- [x] **AC-US2-03**: Missing `externalModels` section is created with defaults
...
**Acceptance Criteria**:
- [x] **AC-US3-01**: `createProvider()` checks consent for external providers
- [x] **AC-US3-02**: Throws `ExternalApiConsentDeniedError` when consent denied
- [x] **AC-US3-03**: Free providers (claude-code, ollama) created without consent check
...
**Acceptance Criteria**:
- [x] **AC-US4-01**: `judge()` checks consent before API call
- [x] **AC-US4-02**: Falls back to `basicEvaluation()` when consent denied
- [x] **AC-US4-03**: Skips consent check when no API key (already falls back)
...
**Acceptance Criteria**:
- [x] **AC-US5-01**: judge-llm SKILL.md includes consent check instructions
- [x] **AC-US5-02**: done SKILL.md references consent before judge-llm step
