...
**Acceptance Criteria**:
- [x] **AC-US1-01**: When the backend classifies an error as `model_not_found`, the ErrorCard displays a distinct icon and color (not the generic "unknown" style)
- [x] **AC-US1-02**: The ErrorCard hint text for `model_not_found` reads: "The selected model is not available. Try switching to a different model in the dropdown above."
- [x] **AC-US1-03**: A shared `ClassifiedError` type definition exists on the frontend that includes `model_not_found` in its category union, matching the backend's canonical definition
- [x] **AC-US1-04**: All existing ErrorCard consumers continue to work without import path changes after the type extraction
...
**Acceptance Criteria**:
- [x] **AC-US2-01**: Clicking the ErrorCard retry button in TestsPanel calls `handleGenerateEvals()` with no arguments (the MouseEvent is not forwarded)
