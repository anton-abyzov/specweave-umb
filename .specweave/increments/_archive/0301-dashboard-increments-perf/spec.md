...
- [x] **AC-US1-03**: `getOverview()` reuses cached increment scan results from `getIncrements()` instead of performing a duplicate filesystem scan
- [x] **AC-US1-04**: Response time for `/api/increments` with 300 increments under 200ms on cache hit, under 2000ms on cold scan
...
**Acceptance Criteria**:
- [x] **AC-US2-01**: `/api/increments` accepts `limit` (default 50, max 500), `offset` (default 0) query parameters
- [x] **AC-US2-02**: `/api/increments` accepts `status` and `type` filter query parameters for server-side filtering
- [x] **AC-US2-03**: Response includes pagination metadata: `{ total, limit, offset, hasMore }`
- [x] **AC-US2-04**: Frontend `IncrementsPage` uses server-side pagination with "load more" or page navigation instead of loading all increments at once
- [x] **AC-US2-05**: Summary counts remain unaffected by pagination (always reflect the full dataset)
...
**Acceptance Criteria**:
- [x] **AC-US3-01**: SSE `increment-update` events are debounced on the frontend (500ms window) to prevent rapid-fire refetches
- [x] **AC-US3-02**: SSE events carry enough context (increment ID, change type) for the frontend to decide whether a refetch is needed
- [x] **AC-US3-03**: Cache invalidation on the server is triggered by SSE file watcher events, ensuring fresh data on next request
...
**Acceptance Criteria**:
- [x] **AC-US4-01**: Unit tests for the TTL cache mechanism (hit, miss, expiry, invalidation)
- [x] **AC-US4-02**: Unit tests for async filesystem scan producing identical results to the original sync scan
- [x] **AC-US4-03**: Unit tests for pagination logic (limit, offset, filtering, summary preservation)
- [x] **AC-US4-04**: Integration test demonstrating cache sharing between `getIncrements()` and `getOverview()`
