...
**Acceptance Criteria**:
- [ ] **AC-US1-01**: The done skill's Step 8 instructs the LLM to call `specweave complete <id>` CLI command instead of directly editing metadata.json
- [ ] **AC-US1-02**: `completeIncrement()` in `status-commands.ts` awaits `LifecycleHookDispatcher.onIncrementDone()` instead of fire-and-forget (void async IIFE)
- [ ] **AC-US1-03**: The fire-and-forget pattern is replaced with a logged-but-non-blocking await pattern that reports hook results to the user
- [ ] **AC-US1-04**: Existing unit tests for `completeIncrement()` continue to pass
...
**Acceptance Criteria**:
- [ ] **AC-US2-01**: A new skill at `plugins/specweave/skills/sync-docs/SKILL.md` exists with proper frontmatter (description, argument-hint)
- [ ] **AC-US2-02**: The skill invokes `LivingDocsSync.syncIncrement()` for the given increment ID
- [ ] **AC-US2-03**: The skill accepts an optional "review" mode argument that validates sync completeness without modifying files
- [ ] **AC-US2-04**: The skill handles errors gracefully and reports sync results (files created/updated count)
...
**Acceptance Criteria**:
- [ ] **AC-US3-01**: `LifecycleHookDispatcher.onIncrementDone()` checks `hooks.post_increment_done.sync_to_github_project` config flag
- [ ] **AC-US3-02**: When enabled, it invokes the GitHub feature sync CLI (`github-feature-sync-cli.js`) for the increment's feature ID
- [ ] **AC-US3-03**: Feature ID is resolved from the increment's spec.md frontmatter or metadata.json
- [ ] **AC-US3-04**: Handler failure does not block other post-closure operations (error-isolated)
- [ ] **AC-US3-05**: Unit test verifies the handler dispatches when flag is true and skips when false
...
**Acceptance Criteria**:
- [ ] **AC-US4-01**: The done skill's Step 9 (Post-Closure Sync) is updated to show the sync result summary table after `specweave complete` runs
- [ ] **AC-US4-02**: If any sync operation fails, the skill shows "Run /sw:progress-sync to retry" guidance
