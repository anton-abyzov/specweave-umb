...
- **2026-03-03**: Default to `gemini-3.1-flash-image-preview` (Nano Banana 2) for image generation unless user explicitly requests Pro quality
- **2026-03-03**: Long-running agent teams must use turn-based idle/timeout detection (check message count, last update per turn) not wall-clock timers - Claude Code processes messages reactively without built-in timer mechanisms for elapsed time measurement
